...
def validate_plan_inputs(
    ...
    if missing:
        errors.append(
            ValidationError("MISSING_NORMS", f"Missing norms for positions: {', '.join(missing)}.")
        )

    for position in demands:
        if position not in priorities:
            errors.append(
                ValidationError("MISSING_PRIORITY", f"Missing priority for position {position!r}.")
            )
        ...
        if position not in assigned_positions:
            ...
            errors.append(
                ValidationError(
                    ...
                    f"units, expected {demands[position]}.",
                )
            )
...
def check_plan(
    ...
    errors = validate_plan_inputs(demands, priorities, norms)
    if errors:
        return errors
    return validate_plan_result(plan_result, demands, shift_hours)
